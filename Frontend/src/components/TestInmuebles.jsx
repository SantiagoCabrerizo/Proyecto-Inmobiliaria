    ...
    <div>
      <table className="table table-hover container mt-5 mb-5">
        ...
        <tbody>
          ...
          {inmueble.map(inmueble => 
            <tr key={inmueble.id}>
              <td>{inmueble.direccion}</td>
              <td>{inmueble.caracteristicas}</td>
              <td>{inmueble.dueño.nombre}</td>
              <td>{inmueble.inquilino}</td>
              <td>{inmueble.tiposInmueble}</td>
              <td>{inmueble.tipoNegocio}</td>
              <td>{inmueble.valor}</td>
              <td>
                <Link to={`/inmueble/${inmueble.id}`}>Detalles</Link>
              </td>
            </tr>
          )}
        </tbody>
    ...
    useEffect(() => {
        InmuebleService.getInmuebleAll()
            .then((res) => {
                ...
                // Después de obtener los datos, crea la URL de la imagen
                const blob = new Blob([res.data.imagenData], { type: 'image/jpeg' });
                ...
                <tbody>
                    {inmueble.content.map((row, index) => (
                        <tr key={index}>
                            ...
                            <td>{row[0].valor}</td>
                            <td>
                                <img id="imagen" alt="Imagen del inmueble" src={imageUrl} />
                            </td>
                            <td>
                        ...
                        <tr key={index}>
                            ...
                            <td>
                                <img id="imagen" alt="Imagen del inmueble" src={URL.createObjectURL(
                                    base64StringToBlob(row[1], 'image/jpeg'))} style={{
